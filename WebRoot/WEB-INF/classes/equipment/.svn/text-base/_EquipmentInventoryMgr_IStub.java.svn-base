	...
	implements equipment.EquipmentInventoryMgr_I
{
	...
	public void getAllSupportedPTPs(globaldefs.NameAndStringValue_T[] equipmentName, terminationPoint.TerminationPointList_THolder tpList) throws globaldefs.ProcessingFailureException
	{
		...
		else
		{
			...
			try
			{
				_localServant.getAllSupportedPTPs(equipmentName,tpList);
			}
			finally
			{
	...
	public void getAllEquipment(globaldefs.NameAndStringValue_T[] meName, equipment.EquipmentOrHolderList_THolder eqList) throws globaldefs.ProcessingFailureException
	{
		...
		else
		{
			...
			try
			{
				_localServant.getAllEquipment(meName,eqList);
			}
			finally
			{
				_servant_postinvoke(_so);
	...
	public void getAllEquipmentNames(globaldefs.NameAndStringValue_T[] meName, globaldefs.NamingAttributesList_THolder nameList) throws globaldefs.ProcessingFailureException
	{
		...
		else
		{
			...
			try
			{
				_localServant.getAllEquipmentNames(meName,nameList);
			}
			finally
			{
				_servant_postinvoke(_so);
	...
	public void getEquipmentVersion(globaldefs.NameAndStringValue_T[][] objNameList, globaldefs.NameAndNVSList_THolder objVersionList) throws globaldefs.ProcessingFailureException
	{
		...
		if(! this._is_local())
		{
			...
			try
			{
				_os = _request( "getEquipmentVersion", true);
				globaldefs.NamingAttributesList_THelper.write(_os,objNameList);
				_is = _invoke(_os);
				objVersionList.value = globaldefs.NameAndNVSList_THelper.read(_is);
				return;
			}
			catch( org.omg.CORBA.portable.RemarshalException _rx ){}
			catch( org.omg.CORBA.portable.ApplicationException _ax )
		...
		{
			org.omg.CORBA.portable.ServantObject _so = _servant_preinvoke( "getEquipmentVersion", _opsClass );
			if( _so == null )
				throw new org.omg.CORBA.UNKNOWN("local invocations not supported!");
			EquipmentInventoryMgr_IOperations _localServant = (EquipmentInventoryMgr_IOperations)_so.servant;
			try
			{
				_localServant.getEquipmentVersion(objNameList,objVersionList);
			}
			finally
			{
				_servant_postinvoke(_so);
